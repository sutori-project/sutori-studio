...
ShowImageResourceDialog(imageElement){var _a;const index=ExtraTools.GetElementIndex(imageElement);const resource=App.Document.Resources[index];const pageHtml=`<div><div><label for="tb-id"class="form">Resource ID</label><input id="tb-id"type="text"class="form"value="${resource.ID}"/></div><div><label for="tb-src"class="form">File Path/URL</label><input id="tb-src"type="text"class="form"value="${(_a = resource.Src) !== null && _a !== void 0 ? _a : ''}"/></div><div style="text-align:right; margin-top:15px;"><a class="form secondary"onclick="App.Dialogs.Close();"style="color:#fff;">Cancel</a><a class="form"onclick="App.Dialogs.Ok();"style="color:#fff;">OK</a></div></div>`;this.OkCallback=function(){const dest=document.getElementById('dialog-wrapper');const dialog=dest.querySelector('dialog');resource.ID=dialog.querySelector('#tb-id').value;resource.Src=dialog.querySelector('#tb-src').value;imageElement.querySelector('.group-id').textContent='#'+resource.ID;this.Close();};this.ShowDialog('Image Resource Properties',pageHtml);}
...
throw new Error("Tried to create more than one instance of SutoriApp");this.WebMode=webMode;this.MainElement=document.querySelector('.app');this.Moments=new MomentFlow;this.Sidebar=new SidebarFlow;this.Dialogs=new DialogFlow;this.Reset();}
...
else{let entries=await Neutralino.os.showOpenDialog('Open Sutori Document',{filters:[{name:'Sutori XML Document',extensions:['xml']},{name:'All files',extensions:['*']}]});if(entries.length>0){const file=entries[0];const data=await Neutralino.filesystem.readFile(file);await self._OpenFileData(data);}}}
...
async SaveAsFile(){const self=App;console.log('save-as clicked');const doc=document.implementation.createDocument(null,'document');self.SerializeDoc(doc);const xml=ExtraTools.StringifyXml(doc);if(self.WebMode){if(typeof window.showSaveFilePicker==='undefined'){alert('Error, your browser does not support the filesystem api.');return;}
const options={types:[{description:'Xml Files',accept:{'application/xml':['.xml']},},],};const fileHandle=await window.showSaveFilePicker(options);const writable=await fileHandle.createWritable();await writable.write(xml);await writable.close();}
else{let file=await Neutralino.os.showSaveDialog('Save Sutori Document',{title:'new_document.xml',filters:[{name:'Sutori XML Document',extensions:['xml']},{name:'All files',extensions:['*']}]});if(file.length>0){await Neutralino.filesystem.writeFile(file,xml);}}
