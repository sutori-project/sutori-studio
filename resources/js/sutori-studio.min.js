...
ShowImageResourceDialog(imageElement){var _a,_b;const index=ExtraTools.GetElementIndex(imageElement);const resource=App.Document.Resources[index];const pageHtml=`<div><div class="row has-gap"><div class="column"><figure><img id="img-preview"src="${(_a = resource.Src) !== null && _a !== void 0 ? _a : ''}"/></figure></div><div class="column"><div><label for="tb-id"class="form">Resource ID</label><input id="tb-id"type="text"class="form"value="${resource.ID}"/></div><div><label for="tb-name"class="form">Resource Name</label><input id="tb-name"type="text"class="form"value="${resource.Name}"/></div><div><label for="tb-src"class="form">File Path/URL</label><div class="input-with-button"><input id="tb-src"type="text"class="form"value="${(_b = resource.Src) !== null && _b !== void 0 ? _b : ''}"/><a id="button-browse"class="form secondary"onclick="App.Dialogs.ImageBrowse('#tb-src', '#tb-name', '#img-preview');">...</a></div></div></div></div><div style="text-align:right; margin-top:15px;"><a class="form"onclick="App.Dialogs.Ok();"style="color:#fff;">OK</a><a class="form secondary"onclick="App.Dialogs.Close();"style="color:#fff;">Cancel</a></div></div>`;this.OkCallback=function(){const dest=document.getElementById('dialog-wrapper');const dialog=dest.querySelector('dialog');resource.ID=dialog.querySelector('#tb-id').value;resource.Src=dialog.querySelector('#tb-src').value;resource.Name=dialog.querySelector('#tb-name').value;imageElement.querySelector('.group-id').textContent='#'+resource.ID;imageElement.querySelector('.group-name').textContent=resource.Name;this.Close();};this.ShowDialog('Image Resource Properties','resource-dialog',pageHtml);}
...
this.Close();};this.ShowDialog('Moment Properties','moment-dialog',pageHtml);}
