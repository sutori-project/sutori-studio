...
this.Close();};this.ShowDialog('Moment Properties',pageHtml);}
ShowDialog(header,html,draggable=false){const dest=document.getElementById('dialog-wrapper');dest.innerHTML=`<dialog><div class="header"><span>${header}</span><a class="close-but"onclick="App.Dialogs.Close();"><svg width="12"height="12"><use xlink:href="#close"/></svg></a></div><div class="body">${html}</div></dialog>`;const dialog=dest.querySelector('dialog');if(draggable===true)
...
AddRow(text,id,actor_id,createDocElement=true){const random_id=actor_id!==null&&actor_id!==void 0?actor_id:ExtraTools.RandomID();if(createDocElement){const moment=new SutoriMoment;moment.ID=random_id;moment.AddText(App.SelectedCulture,text);moment.Actor=actor_id;App.Document.AddMoment(moment);}
...
class SutoriBuilderApp{constructor(){this._culture=SutoriCulture.None;if(typeof globalThis.App!=='undefined')
throw new Error("Tried to create more than one instance of SutoriApp");this.MainElement=document.querySelector('.app');this.Moments=new MomentFlow;this.Sidebar=new SidebarFlow;this.Dialogs=new DialogFlow;this.Reset();}
get SelectedCulture(){return this._culture;}
get Document(){return this._loadedDocument;}
static init(){if(typeof globalThis.App=='undefined'){const app=globalThis.App=new SutoriBuilderApp();const menu=app.MainElement.querySelector('.main-menu');app.AttachEvent('li[action="new"]','click',app.NewFile);app.AttachEvent('li[action="open"]','click',app.OpenFile);app.AttachEvent('li[action="save-as"]','click',app.SaveAsFile);app.AttachEvent('li[action="exit"]','click',app.Exit);app.AttachEvent('li[action="help-website"]','click',app.GotoHelpWebsite);document.addEventListener('keydown',app.HandleKeyDown);document.addEventListener('keyup',app.HandleKeyUp);document.querySelectorAll('li[action="change-lang"]').forEach(li=>{li.addEventListener('click',app.HandleChangeLang);});window.onmouseup=event=>event.preventDefault();console.log('loaded SutoriApp');}}
AttachEvent(selector,event,callback){document.querySelector(selector).addEventListener(event,callback);}
HandleChangeLang(e){const li=e.target;const lang=li.dataset['lang'];App._culture=SutoriCulture[lang];setTimeout(()=>{li.blur();},300);li.parentElement.querySelector('li[checked="true"]').setAttribute('checked',null);li.setAttribute('checked','true');for(var i=0;i<App.Document.Moments.length;i++){const moment=App.Document.Moments[i];const rowElement=App.Moments.MainElement.querySelector('.moment-flow .moment-row:nth-of-type('+(i+1)+')');const momentElement=rowElement.querySelector('.moment');const editor=rowElement.querySelector('.moment-body');editor.innerText=moment.GetText(App._culture);const optionsContainer=rowElement.querySelector('.moment-options');optionsContainer.innerHTML='';moment.GetOptions(App._culture).forEach(option=>{App.Moments.AddOption(momentElement,option.Text);});const mediaContainer=rowElement.querySelector('.moment-media');mediaContainer.innerHTML='';moment.GetImages(App._culture).forEach(image=>{App.Moments.AddImage(momentElement,image.Src);});}}
HandleKeyDown(e){switch(e.key){case'Enter':const target=e.target;if(target.classList.contains('group-name')){target.blur();e.preventDefault();}
break;}}
...
break;case'Delete':const delTarget=e.target;if(delTarget.classList.contains('group')){let button=await Neutralino.os.showMessageBox('Confirm',"Are you sure you wish to delete this?",'OK_CANCEL','QUESTION');if(button=='OK'){const group=delTarget.parentElement.dataset['group'];const index=ExtraTools.GetElementIndex(delTarget);if(index>-1){switch(group){case'includes':self.Document.Includes.splice(index,1);break;case'actors':self.Document.Actors.splice(index,1);break;}
...
enterEle.click();break;default:console.log(e);break;}}
async NewFile(){let button=await Neutralino.os.showMessageBox('New Document',"Are you sure you wish to do this?",'OK_CANCEL','QUESTION');if(button=='OK'){App.Reset();}}
async OpenFile(){const self=App;console.log('open clicked');let entries=await Neutralino.os.showOpenDialog('Open Sutori Document',{filters:[{name:'Sutori XML Document',extensions:['xml']},{name:'All files',extensions:['*']}]});if(entries.length>0){const file=entries[0];const dir=file.substring(0,file.lastIndexOf('/')+1);self.Reset();const data=await Neutralino.filesystem.readFile(file);const doc=self._loadedDocument=new SutoriDocument();doc.CustomUriLoader=async function(uri){return'';};await doc.AddDataFromXml(data);doc.Includes.forEach((include)=>{const includeElement=self.Sidebar.AddInclude(include.Path,false);});doc.Actors.forEach((actor)=>{let color='';if(typeof actor.Attributes['color']!=="undefined"){color=actor.Attributes['color'];}
const actorElement=self.Sidebar.AddActor(actor.Name,color,false);});doc.Moments.forEach((moment)=>{const text=moment.GetText(self._culture);const momentElement=self.Moments.AddRow(text,moment.ID,moment.Actor,false);const optionsContainer=momentElement.querySelector('.moment-options');optionsContainer.innerHTML='';moment.GetOptions(App._culture).forEach(option=>{App.Moments.AddOption(momentElement,option.Text);});const mediaContainer=momentElement.querySelector('.moment-media');mediaContainer.innerHTML='';moment.GetImages(App._culture).forEach(image=>{App.Moments.AddImage(momentElement,image.Src);});});return;console.log("Loaded!",self._loadedDocument);}}
async SaveAsFile(){const self=App;console.log('save-as clicked');let file=await Neutralino.os.showSaveDialog('Save Sutori Document',{title:'new_document.xml',filters:[{name:'Sutori XML Document',extensions:['xml']},{name:'All files',extensions:['*']}]});if(file.length>0){const doc=document.implementation.createDocument(null,'document');self.SerializeDoc(doc);const xml=ExtraTools.StringifyXml(doc);await Neutralino.filesystem.writeFile(file,xml);console.log("Saved!",file);alert('Saved!');}}
...
else if(element instanceof SutoriElementImage){const image=element;const ie=momentElement.appendChild(doc.createElement('image'));ie.textContent=image.Src;if(image.ContentCulture!==SutoriCulture.None)
ie.setAttribute('lang',image.ContentCulture);if(!ExtraTools.IsEmptyString(image.Actor))
ie.setAttribute('actor',image.Actor);if(!ExtraTools.IsEmptyString(image.For))
ie.setAttribute('for',image.For);if(image.Preload===true)
