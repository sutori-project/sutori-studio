...
else{const filename=uri.includes(':')?uri:App.CurrentDirectory+uri;const file=await Neutralino.filesystem.readBinaryFile(filename);const blob=new Blob([new Uint8Array(file,0,file.length)]);const fr=new FileReader();fr.readAsDataURL(blob);fr.onloadend=function(){previewTarget.src=fr.result;};}}}
...
static async GenerateThumbnail(uri,maxWidth=128){let blob=null;if(ExtraTools.IsEmptyString(uri)||uri.includes('://')||uri.includes('data:image')){const w_response=await fetch(uri);blob=await w_response.blob();}
else{const filename=uri.includes(':')?uri:App.CurrentDirectory+uri;const file=await Neutralino.filesystem.readBinaryFile(filename);blob=new Blob([new Uint8Array(file,0,file.length)]);}
const orig_src=await ExtraTools.LoadImageDataUriFromBlob(blob);const orig_img=await ExtraTools.LoadImage(orig_src);const thumb_canvas=document.createElement('canvas');const ctx=thumb_canvas.getContext('2d');const scale=maxWidth/orig_img.width;const maxHeight=orig_img.height*scale;thumb_canvas.width=maxWidth;thumb_canvas.height=maxHeight;ctx.drawImage(orig_img,0,0,maxWidth,maxHeight);const thumb=document.createElement('img');thumb.src=thumb_canvas.toDataURL();return thumb;}
static async LoadImage(src){return new Promise((resolve,reject)=>{const img=new Image();img.onload=()=>resolve(img);img.onerror=reject;img.src=src;});}
static async LoadImageDataUriFromBlob(blob){return await new Promise((resolve,reject)=>{const fr=new FileReader();fr.onloadend=function(){resolve(fr.result);};fr.readAsDataURL(blob);});}
...
AddActor(name="Untitled",id,color,createDocElement=true){const random_id=ExtraTools.IsEmptyString(id)?ExtraTools.RandomID():id;const color_h=ExtraTools.IsEmptyString(color)?'#FF6263':color;const xml=`<li tabindex="0"class="group has-icon"style="--bg: ${color_h}">`+`<svg class="icon"width="16"height="16"><use xlink:href="#avatar"/></svg>`+`<span class="group-name"onclick="App.Sidebar.HandleGroupNameClick(event);"contenteditable="false">${name}</span><small class="group-id">#${random_id}</small>`+`<a class="button"onclick="App.Dialogs.ShowActorDialog(this.parentElement)">`+`<svg width="12"height="12"><use xlink:href="#cog"/></svg></a>`+`</li>`;const parent=this.MainElement.querySelector('.groups.actors');parent.innerHTML+=xml;if(createDocElement){const actor=new SutoriActor();actor.Name=name;actor.ID=random_id;App.Document.Actors.push(actor);}
return parent.querySelector('li:last-of-type');}
...
class SutoriBuilderApp{constructor(webMode){this._culture=SutoriCulture.None;this._splitting=false;if(typeof globalThis.App!=='undefined')
throw new Error("Tried to create more than one instance of SutoriApp");this.WebMode=webMode;this.MainElement=document.querySelector('.app');this.Moments=new MomentFlow;this.Sidebar=new SidebarFlow;this.Dialogs=new DialogFlow;this.CurrentDirectory='';this.CurrentFilename='';this.CurrentName='';this._loadedDocument=new SutoriDocument;this._thumbs=new Map();this.Sidebar.Reset();this.Moments.Reset();}
get SelectedCulture(){return this._culture;}
get Document(){return this._loadedDocument;}
static init(webMode=false){if(typeof globalThis.App=='undefined'){const app=globalThis.App=new SutoriBuilderApp(webMode);const menu=app.MainElement.querySelector('.main-menu');app.AttachEvent('li[action="new"]','click',app.NewFile);app.AttachEvent('li[action="open"]','click',app.OpenFile);app.AttachEvent('li[action="save-as"]','click',app.SaveAsFile);app.AttachEvent('li[action="exit"]','click',app.Exit);app.AttachEvent('li[action="help-website"]','click',app.GotoHelpWebsite);document.addEventListener('keydown',app.HandleKeyDown);document.addEventListener('keyup',app.HandleKeyUp);document.addEventListener('mousemove',app.SplitterMove);app.AttachEvent('.mid-pane .splitter','mousedown',app.SplitterDown);app.AttachEvent('.mid-pane','mouseup',app.SplitterUp);const body=document.body;body.addEventListener('contextmenu',app.HandleContextMenu);document.querySelectorAll('li[action="change-lang"]').forEach(li=>{li.addEventListener('click',app.HandleChangeLang);});window.onmouseup=event=>event.preventDefault();console.log('loaded SutoriApp');}}
...
async NewFile(){const message='Are you sure you wish to do this?';console.log('new clicked',App.WebMode);if(App.WebMode==true){if(confirm(message)==true){await App.Reset();await App.SetCurrentFile("");}}
else{let button=await Neutralino.os.showMessageBox('New Document',message,'OK_CANCEL','QUESTION');if(button=='OK'){await App.Reset();await App.SetCurrentFile("");}}}
...
else{let entries=await Neutralino.os.showOpenDialog('Open Sutori Document',{filters:[{name:'Sutori XML Document',extensions:['xml']},{name:'All files',extensions:['*']}]});if(entries.length>0){const file=entries[0];await App.SetCurrentFile(file);const data=await Neutralino.filesystem.readFile(file);await self._OpenFileData(data);}}}
async _OpenFileData(data){const self=App;await self.Reset();const doc=self._loadedDocument=new SutoriDocument();doc.CustomUriLoader=async function(uri){return'';};await doc.AddDataFromXml(data);doc.Includes.forEach((include)=>{const includeElement=self.Sidebar.AddInclude(include.Path,false);});doc.Actors.forEach((actor)=>{let color='';if(typeof actor.Attributes['color']!=="undefined"){color=actor.Attributes['color'];}
const actorElement=self.Sidebar.AddActor(actor.Name,actor.ID,color,false);});doc.Moments.forEach(async(moment)=>{const text=moment.GetText(self._culture);const momentElement=self.Moments.AddRow(text,moment.ID,moment.Actor,false);const optionsContainer=momentElement.querySelector('.moment-options');optionsContainer.innerHTML='';moment.GetOptions(App._culture).forEach(option=>{App.Moments.AddOption(momentElement,option.Text);});const mediaContainer=momentElement.querySelector('.moment-media');mediaContainer.innerHTML='';moment.GetImages(App._culture).forEach(async(image)=>{let src='';if(!ExtraTools.IsEmptyString(image.ResourceID)){const resource=doc.GetResourceByID(image.ResourceID);if(resource instanceof SutoriResourceImage){src=await App.GetThumbnailDataUri(resource.ID,resource.Src);}}
App.Moments.AddImage(momentElement,src);});});doc.Resources.forEach((resource)=>{if(resource instanceof SutoriResourceImage){const resourceElement=self.Sidebar.AddImageResource(resource.Name,resource.ID,false);}});}
...
else{let file=await Neutralino.os.showSaveDialog('Save Sutori Document',{title:'new_document.xml',defaultPath:'untitled.xml',filters:[{name:'Sutori XML Document',extensions:['xml']},{name:'All files',extensions:['*']}]});if(file.length>0){await Neutralino.filesystem.writeFile(file,xml);await App.SetCurrentFile(file);saved=true;}}
...
async Reset(){const self=this;self._loadedDocument=new SutoriDocument;self.Sidebar.Reset();self.Moments.Reset();}
GotoHelpWebsite(){Neutralino.os.open('https://kodaloid.com');}
Exit(){Neutralino.app.exit();}
async SetCurrentFile(filename){console.log(filename);const self=this;const name=filename.split('\\').pop().split('/').pop();const path=filename.replace(name,'');self.CurrentDirectory=path;self.CurrentFilename=filename;self.CurrentName=name;const title=ExtraTools.IsEmptyString(name)?'Untitled':name;await Neutralino.window.setTitle(`[${title}]-Sutori Studio`);}
async GetThumbnailDataUri(key,uri){if(!this._thumbs.has(key)){this._thumbs[key]=await ExtraTools.GenerateThumbnail(uri);}
const img=this._thumbs[key];return img.src;}
SplitterDown(e){App._splitting=true;}
SplitterUp(e){App._splitting=false;}
SplitterMove(e){if(App._splitting){let newX=e.clientX;if(newX<200)
newX=200;if(newX>500)
newX=500;document.getElementById('sidebar').style.width=newX+'px';}}};